...
export async function GET(request: NextRequest) {
  try {
    ...
    // Get provider's completed and cancelled orders
    ...
    // Calculate total earned (only completed orders with final price)
    const totalFaturado =
      orders
        .filter(
    ...
    const stats = {
      ...
      concluidos,
      cancelados,
      totalFaturado: Math.round(totalFaturado * 100) / 100, // Round to 2 decimal places
      avaliacaoMedia: Math.round(avaliacaoMedia * 10) / 10, // Round to 1 decimal place
    };

    return NextResponse.json(stats);
  } catch (error) {
    console.error("Error fetching provider service stats:", error);
