...
export default function TornarPrestadorPage() {
  ...
  const [existingRequest, setExistingRequest] = useState<ProviderRequest | null>(null);
    ...
    async function checkExistingRequest() {
      try {
        ...
        if (response.ok) {
          ...
          const pending = requests.find((req: ProviderRequest) => req.status === "PENDING");
      ...
      <SidebarInset>
        ...
        <div className="flex flex-1 flex-col gap-4 p-4 pt-0">
          <div className="container mx-auto max-w-3xl">
            <Card>
              <CardHeader>
                <CardTitle className="text-2xl">Tornar-se um Prestador de Serviços</CardTitle>
                <CardDescription>
                  Preencha o formulário abaixo para solicitar seu cadastro como prestador de serviços
                </CardDescription>
              </CardHeader>
              <CardContent>
                {existingRequest ? (
                  <div className="space-y-4">
                    <div className="flex items-center gap-2 p-4 bg-amber-50 border border-amber-200 rounded-md">
                      ...
                      <div className="flex-1">
                        <p className="font-medium text-amber-900">Solicitação em Análise</p>
                        <p className="text-sm text-amber-700">
                          Você já possui uma solicitação pendente enviada em{" "}
                          {new Date(existingRequest.createdAt).toLocaleDateString("pt-BR")}
                        </p>
                      </div>
                    ...
                    <p className="text-sm text-muted-foreground">
                      Sua solicitação está sendo analisada por nossa equipe. Você receberá uma notificação
                  ...
                  <>
                    <div className="mb-6 p-4 bg-blue-50 border border-blue-200 rounded-md">
                      <p className="text-sm text-blue-900">
                        <strong>Atenção:</strong> Após enviar sua solicitação, nossa equipe irá analisar
                        suas informações. Você será notificado sobre a decisão.
                      </p>
                    </div>
                    <ProviderRequestForm />
                  </>
